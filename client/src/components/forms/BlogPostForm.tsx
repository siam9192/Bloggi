...
function BlogPostForm() {
  ...
  const handleSubmission = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // Get form values
    const { title, short_description, category, publish_status, privacy_status } = getFormValues(
      e.currentTarget,
      ["title", "short_description", "category", "publish_status", "privacy_status"],
    );
    ...
    if (!featuredPhoto || !featuredPhoto.type.includes("image")) {
      console.log(featuredPhoto);
      errors.featured_photo = "Featured photo is required";
    }
    ...
    try {
      ...
      let featured_image = response.data.data.display_url;
      const payload = {
        title,
        short_description,
        ...
        privacy_status,
        publish_date: scheduledDateTime || new Date(),
        is_premium: false,
      };
      ...
      <form onSubmit={handleSubmission}>
        <div className=" lg:grid grid-cols-6 gap-3">
          <div className="bg-white rounded-md  md:p-5 p-2 h-fit col-span-4">
            <RichEditor onChange={handelEditorOnChange} />
          ...
          <div className=" md:p-5 p-2 rounded-md bg-white h-fit col-span-2 space-y-4">
            ...
            <div>
              {featuredPhoto ? (
                ...
                <div>
                  <img src={URL.createObjectURL(featuredPhoto)} alt="" />
            ...
            {shouldShowDatePicker && <DateTimePicker onChange={handelScheduleOnChange} />}
