...
import ManagePlanTableCard from "../cards/ManagePlanTableCard";
import { useGetPlansForManageQuery } from "@/redux/features/plan/plan.api";
...
const ManagePlans = () => {
  ...
  const { data, isLoading, isFetching } = useGetPlansForManageQuery(params);
  ...
  return (
    <div className="md:mt-10 mt-6 md:p-5 p-3 bg-white overflow-y-auto ">
      <div className="py-4  flex md:flex-row flex-col-reverse  gap-4 justify-between md:items-center">
        ...
        <button className="px-6 py-2 border rounded-md hover:bg-secondary_color">
          Add New Plan
        </button>
      </div>
      <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          {!isLoading || !isFetching
            ? plans?.map((plan, index) => <ManagePlanTableCard plan={plan} key={index} />)
