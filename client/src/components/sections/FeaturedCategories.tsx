...
const getStyle = (imageUrl: string): CSSProperties => {
  return {
    width: "100%",
    height: "250px",
    backgroundImage: `linear-gradient(to top right, rgba(0,0,1,0.7) 10%, rgba(0,0,1,0.1)90%),url(${imageUrl})`,
...
async function FeaturedCategories() {
  ...
  return (
    <div className="py-20">
      <h1 className="text-3xl md:text-4xl font-jost font-extrabold">Featured Categories</h1>
      <div className="mt-10 grid lg:grid-cols-3 md:grid-cols-3  grid-cols-2 gap-5 ">
        {categories?.map((category, index) => (
          <Link href={""} key={"popular-category-" + index}>
            <div style={getStyle(category.image_url!)}>
