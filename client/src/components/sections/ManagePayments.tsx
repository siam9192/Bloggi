...
import ManagePaymentTableCard from "../cards/ManagePaymentTableCard";
import { useGetPaymentsForManageQuery } from "@/redux/features/payment/payment.api";

const heads = ["Trx ID", "Amount", "status", "Method", "Created At", "User", "Actions"];
...
const ManagePayments = () => {
  ...
  const { data, isLoading, isFetching } = useGetPaymentsForManageQuery(params);
    ...
    <div className="md:mt-10 mt-6 md:p-5 p-3 bg-white overflow-y-auto ">
        ...
        <tbody>
          {!isLoading || !isFetching
            ? payments?.map((payment, index) => (
                <ManagePaymentTableCard payment={payment} key={index} />
              ))
      ...
      {!isLoading && !payments?.length && (
        <div className="h-[30vh] text-center">
          <h1 className="text-3xl text-gray-700 font-medium mt-20">No payments found</h1>
        </div>
      )}
      <div className="mt-10 pl-10">
        {!isLoading && meta && payments?.length ? (
